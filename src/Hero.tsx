    ...
    if (Array.isArray(word)) {
      return (
        <motion.span className="inline-flex gap-2 xs:gap-4">
          {word.map((part, idx) => (
            <motion.span key={idx} className="inline-block font-black text-shadow-glow">{part}</motion.span>
          ))}
        </motion.span>
      );
    }
      ...
      <motion.span className="inline-block">
        {word.split('').map((letter, index) => {
          ...
          return (
            <motion.span
              key={`${word}-${index}`}
              className={`inline-block font-black text-shadow-glow ${isLastN ? 'cursor-pointer' : ''}`}
            >
              {letter}
              {isLastN && (
                <motion.span
              ...
              {showSayIt ? (
                ...
                >
                  {words.map((word, i) => (
                    ...
                    >
                      <h1 className="text-3xl xs:text-4xl sm:text-5xl md:text-7xl font-black gradient-text select-none leading-[1.2] xs:leading-[1.3] hover:scale-105 transition-transform duration-300">
